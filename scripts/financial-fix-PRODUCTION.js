...
/**
 * ============================================================================
 * FINANCIAL SYSTEM FIX - VERSÃO PRODUÇÃO (BLINDADA)
 * ============================================================================
 * 
 * ✅ AJUSTES DE SEGURANÇA:
 * 1. Pagamentos zero → CANCELADOS (não deletados) - mantém histórico
 * 2. Duplicados → Mantém 'paid' se existir, senão o mais recente
 * 3. Appointments duplicados → Prioridade: completed > scheduled > canceled
 * 4. Overused → Logado antes de corrigir
 * 
 * Uso: 
 *   node financial-fix-PRODUCTION.js --dry-run     # Simulação
 *   node financial-fix-PRODUCTION.js --apply       # EXECUÇÃO REAL
 * ============================================================================
 */
...
async function initModels() {
    ...
    const appointmentSchema = new mongoose.Schema({
        patient: { type: mongoose.Schema.Types.ObjectId },
        patientInfo: { fullName: String, phone: String },
        doctor: { type: mongoose.Schema.Types.ObjectId },
...
class SmartAnalyzer {
    constructor() {
        this.report = {
            paidNoPayment: { total: 0, particular: [], package: [], convenio: [], desconhecido: [] },
            duplicatePayments: [],
            zeroPayments: [],
    ...
    async analyzeDuplicatePayments() {
        ...
        for (const [apptId, pays] of Object.entries(byAppointment)) {
            ...
            if (active.length > 1) {
                // Ordenar: paid primeiro, depois por data (mais recente)
                ...
                this.report.duplicatePayments.push({
                    ...
                    count: active.length,
                    hasPaid: active.some(p => p.status === 'paid'),
    ...
    async analyzeZeroPayments() {
        ...
        const payments = await Payment.find({ amount: { $lte: 0 }, status: { $ne: 'canceled' } }).lean();
    ...
    async analyzeDuplicateAppointments() {
        ...
        const appointments = await Appointment.find({ operationalStatus: { $ne: 'canceled' } }).lean();
        ...
        for (const [key, group] of Object.entries(byKey)) {
            if (group.length > 1) {
                // Prioridade: completed > scheduled > others
                const priority = { 'completed': 3, 'scheduled': 2, 'confirmed': 2, 'paid': 2 };
                const sorted = group.sort((a, b) => {
                    const pa = priority[a.operationalStatus] || 1;
                    const pb = priority[b.operationalStatus] || 1;
                ...
                this.report.duplicateAppointments.push({
                    key,
                    count: group.length,
                    hasCompleted: group.some(a => a.operationalStatus === 'completed'),
                    ids: sorted.map(a => ({ id: a._id.toString(), status: a.operationalStatus })),
                    keep: sorted[0]._id.toString(),
                    cancel: sorted.slice(1).map(a => a._id.toString())
                });
            }
        }
    ...
    async analyzeOverusedPackages() {
        ...
        const packages = await Package.find({ $expr: { $gt: ['$sessionsDone', '$totalSessions'] } }).lean();
        
        this.report.overusedPackages = await Promise.all(packages.map(async p => {
            const patient = await Patient?.findById(p.patient).select('fullName').lean();
            return {
                id: p._id.toString(),
                ...
                diff: p.sessionsDone - p.totalSessions,
                type: p.type,
                status: p.status
            };
        }));
        ...
        for (const pkg of this.report.overusedPackages) {
            console.log(`    ⚠️  ${pkg.id}: ${pkg.sessionsDone}/${pkg.totalSessions} (+${pkg.diff})`);
        }
    }
...
class ProductionFixer {
    ...
    async fix(report) {
        console.log('\n' + '='.repeat(80));
        console.log(`EXECUTANDO CORREÇÕES ${this.dryRun ? '(DRY RUN)' : '🔴 PRODUÇÃO'}`);
    ...
    // ============================================================================
    // 1. CORRIGIR PAGAMENTOS ZERO → CANCELAR (NÃO DELETAR)
    // ============================================================================
    async fixZeroPayments(zeroPayments) {
        console.log('\n🛠️  Corrigindo pagamentos zero (CANCELANDO, não deletando)...');
        
        for (const p of zeroPayments) {
            const action = {
                type: 'CANCEL_ZERO_PAYMENT',
                id: p.id,
                amount: p.amount,
                reason: 'Valor zero ou negativo - cancelado para auditoria'
            };

            if (this.dryRun) {
                console.log(`  [DRY RUN] Cancelar payment ${p.id} (valor: ${p.amount})`);
            } else {
                await Payment.findByIdAndUpdate(p.id, {
                    status: 'canceled',
                    canceledAt: new Date(),
                    canceledReason: 'Valor zero ou negativo - cancelado por auditoria'
                });
                console.log(`  ✅ Cancelado payment ${p.id}`);
            }
    ...
    // ============================================================================
    // 2. CORRIGIR PAGAMENTOS DUPLICADOS → MANTÉM 'paid' SE EXISTIR
    // ============================================================================
    async fixDuplicatePayments(duplicates) {
        console.log('\n🛠️  Corrigindo pagamentos duplicados (mantém PAID se existir)...');
        
        for (const dup of duplicates) {
            const sorted = dup.payments; // Já ordenado: paid primeiro, depois mais recente
            const keep = sorted[0];
            const cancel = sorted.slice(1);

            console.log(`\n  Appointment ${dup.appointmentId}:`);
            console.log(`    Manter: ${keep.id} (R$ ${keep.amount}, status: ${keep.status})`);
            if (dup.hasPaid) console.log(`    ✓ Priorizando payment 'paid'`);
            ...
            for (const p of cancel) {
                ...
                if (this.dryRun) {
                    console.log(`    [DRY RUN] Cancelar: ${p.id} (status: ${p.status})`);
                } else {
                    await Payment.findByIdAndUpdate(p.id, {
                        status: 'canceled',
                        canceledAt: new Date(),
    ...
    // ============================================================================
    // 4. CORRIGIR PACOTES OVERUSED → COM LOG DETALHADO
    // ============================================================================
    async fixOverusedPackages(overused) {
        console.log('\n🛠️  Corrigindo pacotes overused (com log detalhado)...');
        ...
        for (const pkg of overused) {
            const action = {
                ...
                before: pkg.sessionsDone,
                after: pkg.totalSessions,
                diff: pkg.diff,
                reason: 'Correção automática - mais sessões que o total permitido'
            };

            console.log(`\n  Pacote ${pkg.id} (Paciente: ${pkg.patientName}):`);
            console.log(`    ❌ BEFORE: ${pkg.sessionsDone} sessões (total: ${pkg.totalSessions})`);
            console.log(`    ✅ AFTER:  ${pkg.totalSessions} sessões`);
            console.log(`    ⚠️  Diferença: ${pkg.diff} sessões a mais`);
    ...
    // ============================================================================
    // 5. CORRIGIR APPOINTMENTS DUPLICADOS → PRIORIDADE: completed > scheduled
    // ============================================================================
    async fixDuplicateAppointments(duplicates) {
        console.log('\n🛠️  Corrigindo appointments duplicados (prioridade: completed > scheduled)...');
        
        for (const dup of duplicates) {
            console.log(`\n  Duplicata ${dup.key}:`);
            console.log(`    Total: ${dup.count} appointments`);
            if (dup.hasCompleted) console.log(`    ✓ Detectado 'completed' - será mantido`);
            
            console.log(`    Manter: ${dup.keep}`);

            for (const id of dup.cancel) {
                const action = {
    ...
    async reportPaidNoPayment(paidNoPayment) {
        ...
        if (paidNoPayment.particular.length > 0) {
            ...
            console.log(`   ⚠️  ANÁLISE MANUAL NECESSÁRIA antes de criar payments!`);
        }
...
async function main() {
    ...
    console.log('='.repeat(80));
    console.log('FINANCIAL SYSTEM FIX - VERSÃO PRODUÇÃO');
    console.log('='.repeat(80));
    console.log(`Data: ${moment().tz(TIMEZONE).format('YYYY-MM-DD HH:mm:ss')}`);
    console.log(`Modo: ${dryRun ? 'DRY RUN / ANÁLISE' : '🔴 EXECUÇÃO REAL'}`);
    console.log('='.repeat(80) + '\n');

    if (!dryRun) {
        console.log('⚠️  ⚠️  ⚠️  ATENÇÃO  ⚠️  ⚠️  ⚠️');
        console.log('Você está prestes a MODIFICAR o banco de dados em PRODUÇÃO!');
        console.log('');
        console.log('GARANTIAS DESTE SCRIPT:');
        console.log('  ✓ NUNCA cria payments automaticamente');
        console.log('  ✓ NUNCA deleta dados (apenas cancela)');
        console.log('  ✓ MANTÉM histórico completo');
        console.log('  ✓ Prioriza pagamentos PAID em duplicatas');
        console.log('  ✓ Prioriza appointments COMPLETED em duplicatas');
        console.log('');
    ...
    try {
        ...
        console.log(`\n📄 Análise salva em: ${reportFile}`);
        
        if (analyze) {
            console.log('\n✅ Análise completa. Verifique o arquivo JSON.');
            return;
        }
        
        const fixer = new ProductionFixer(dryRun);
        ...
        if (!dryRun) {
            ...
            console.log('='.repeat(80));
            console.log('\nPróximos passos:');
            console.log('1. Verifique o log de correções');
            console.log('2. Analise manualmente os casos PARTICULAR');
            console.log('3. Rode o daily-closing para validar');
            console.log('4. Monitore o sistema por 24-48h');
        }
