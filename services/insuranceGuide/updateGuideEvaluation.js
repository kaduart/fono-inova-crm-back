...
export async function updateGuideEvaluation(guide, payload, user) {
  ...
  await runTransactionWithRetry(async mongoSession => {
    ...
    const doctorChanged = guide.doctorId != null && String(guide.doctorId) !== String(appointment.doctor || '');
    if (!scheduleChanged && !valueChanged && !doctorChanged) return;
    ...
    if (scheduleChanged || doctorChanged) await checkSlotConflicts({ slots: [{ dateStr: new Date(date).toISOString().substring(0, 10), time }], doctorId: doctorChanged ? guide.doctorId : appointment.doctor, patientId: appointment.patient, duration: appointment.duration || 40, mongoSession, excludeAppointmentIds: [appointment._id] });
    const update = applyFinancialProtection(appointment, {
      ...
      ...(doctorChanged ? { doctor: guide.doctorId } : {})
    });
