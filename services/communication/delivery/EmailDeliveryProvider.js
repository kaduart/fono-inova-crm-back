...
export class EmailDeliveryProvider {
   ...
   * @param {string} context.communicationId
  ...
  async executeDelivery(jobData) {
    ...
    if (!destination) {
      return { success: false, error: new Error('Destinatário não informado e convênio não possui e-mail padrão') };
    }

    const pkg = await CommunicationPackage.findOne({ communicationId }).lean();
    if (!pkg || pkg.attachments.length === 0) {
      return { success: false, error: new Error('Pacote de envio não possui documentos') };
    }
    ...
    if (requiredDocumentTypes.length > 0) {
      ...
      if (!validation.valid) {
        return { success: false, error: new Error(`Documentos obrigatórios pendentes: ${validation.missing.join(', ')}`) };
      }
    }
