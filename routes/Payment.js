...
// 🔥 PATCH /:id - VERSÃO OTIMIZADA (SEM TRANSACTION, SEM RETRY LOOP)
router.patch('/:id', auth, async (req, res) => {
    ...
    try {
        ...
        if (payment.package && status !== undefined) {
            essentialUpdates.push(
                Session.updateMany(
                    ...
                    { $set: { isPaid: status === 'paid', status: status === 'paid' ? 'completed' : 'pending', updatedAt: currentDate } }
                )
            );
        }
        ...
        const appointmentIds = [payment.appointment, ...(payment.advanceSessions?.map(a => a.appointment) || [])].filter(id => id);
        if (appointmentIds.length > 0 && status !== undefined) {
            essentialUpdates.push(
                Appointment.updateMany(
                    { _id: { $in: appointmentIds } },
                    { $set: { paymentStatus: status === 'paid' ? 'paid' : 'pending', operationalStatus: status === 'paid' ? 'confirmed' : 'pending', updatedAt: currentDate } }
                )
            );
        }
        ...
        setImmediate(async () => {
            try {
                ...
                if (advanceServices.length > 0) {
                    for (const sessionData of advanceServices) {
                        try {
                            const newPayment = await Payment.create({
                                patient: payment.patient, doctor: payment.doctor,
                                serviceType: sessionData.serviceType || 'individual_session',
                                amount: sessionData.amount, paymentMethod: paymentMethod || payment.paymentMethod,
                                status, specialty: sessionData.sessionType,
                                sessionType: normalizeSessionType(sessionData.sessionType),
                                isAdvance: true, serviceDate: sessionData.date,
                            ...
                            const newAppointment = await Appointment.create({
                                date: sessionData.date, time: sessionData.time,
                                patient: payment.patient, doctor: payment.doctor,
                                specialty: sessionData.sessionType, serviceType: sessionData.serviceType || 'individual_session',
                                operationalStatus: 'scheduled', clinicalStatus: 'pending',
                                paymentStatus: status === 'paid' ? 'paid' : 'pending',
                                paymentMethod: paymentMethod || payment.paymentMethod,
                                sessionValue: sessionData.amount, payment: newPayment._id,
                                createdAt: currentDate, updatedAt: currentDate,
                            });
                            
                            const newSession = await Session.create({
                                date: sessionData.date, time: sessionData.time,
                                sessionType: sessionData.sessionType, sessionValue: sessionData.amount,
                                patient: payment.patient, doctor: payment.doctor,
                                paymentMethod: paymentMethod || payment.paymentMethod,
                                isAdvance: true, payment: newPayment._id, status: 'scheduled',
                                isPaid: status === 'paid', paymentStatus: status === 'paid' ? 'paid' : 'pending',
                            ...
                            await Appointment.updateOne({ _id: newAppointment._id }, { $set: { session: newSession._id } });
                            await Payment.updateOne({ _id: newPayment._id }, { $set: { appointment: newAppointment._id, session: newSession._id } });
                        } catch (advErr) {
                            console.error('[Payment BG] ❌ AdvanceService erro:', advErr.message);
                        }
                    }
                    await invalidateDailyClosingCache(moment.tz(TIMEZONE).format('YYYY-MM-DD'), 'default').catch(() => {});
                }
                
                await invalidateCacheForPayment(payment).catch(() => {});
            } catch (bgErr) {
                console.error('[Payment BG] ❌ Erro:', bgErr.message);
            }
        });
        ...
        return res.status(500).json({ success: false, message: 'Erro interno ao atualizar pagamento', error: error.message });
    }
});
...
router.get("/daily-closing", async (req, res) => {
    try {
        ...
        const [appointmentsCreated, appointmentsToday, payments] = await Promise.all([
            // 1️⃣ Agendamentos CRIADOS hoje (para pacotes vendidos hoje)
            Appointment.find({
            ...
            // 3️⃣ Pagamentos recebidos hoje (para o Caixa)
            Payment.find({
        ...
        // 🧮 CÁLCULOS FINAIS
        // ======================================================
        // ✅ Usa a timeline que tem o paidStatus calculado
        const timelineAppointments = report.timelines.appointments || [];
        const validAppointments = timelineAppointments.filter(
            (a) => !isCanceled(a.operationalStatus)
        );
        
        // Receita Prevista = todos os appointments (inclusive cancelados)
        report.financial.totalExpected = timelineAppointments.reduce(
            (sum, a) => sum + (a.sessionValue || 0),
            0
        );

        // A Receber = confirmados não pagos EXCETO convênio (que paga depois)
        report.financial.totalRevenue = validAppointments
            .filter((a) => a.paidStatus === "Pendente" && isConfirmed(a.operationalStatus) && !a.isConvenio)
            .reduce((sum, a) => sum + (a.sessionValue || 0), 0);

        report.summary.appointments.pendingCount = validAppointments.filter(
            (a) => a.paidStatus === "Pendente" && isConfirmed(a.operationalStatus) && !a.isConvenio
        ).length;

        // Receita Prevista para o summary = todos os appointments (inclusive cancelados)
        report.summary.appointments.expectedValue = timelineAppointments.reduce(
            (sum, a) => sum + (a.sessionValue || 0),
            0
        );
...
router.get('/insurance/receivables', auth, async (req, res) => {
    try {
        ...
        const insuranceStatuses = status
            ? [status]
            : month
        ...
        const paymentReceivables = await Payment.aggregate([
            ...
            },
            {
                $addFields: {
                    ...
                    _resolvedProvider: {
                        $ifNull: [
                            { $arrayElemAt: ['$packageInfo.insuranceProvider', 0] },
                            '$insurance.provider'
                        ]
                    },
            ...
            // Filtro de provider após resolução (aplica sobre o provider real)
            ...(provider ? [{ $match: { _resolvedProvider: provider } }] : []),
            {
                $group: {
                    _id: { provider: '$_resolvedProvider', patient: '$patient', patientName: '$patientName' },
...
router.post('/balance/:patientId/payment-multi', auth, async (req, res) => {
    ...
    try {
        ...
        validDebitIds.forEach(debitId => {
            ...
            if (debit) {
                debitValues[debitId] = debit.amount;
                ...
                if (debit.paymentId) paidPaymentIds.push(debit.paymentId.toString());
            }
        });
        ...
        try {
            ...
            // Atualizar cada débito individualmente
            for (const debitId of validDebitIds) {
                const debitAmount = debitValues[debitId];
                await PatientBalance.updateOne(
                    { 
                        patient: patientId,
                        'transactions._id': new mongoose.Types.ObjectId(debitId)
                    },
                    {
                        $set: {
                            'transactions.$.isPaid': true,
                            'transactions.$.paidAmount': debitAmount,
                            'transactions.$.paidAt': now,
            ...
            // Atualizar saldo e adicionar transação de pagamento
