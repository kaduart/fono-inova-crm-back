...
/**
 * @route   GET /api/cashflow/historico
 * @desc    Histórico de fluxo de caixa dos últimos N meses
 * @query   ?meses=6 (padrão: 6 meses)
 */
router.get('/historico', auth, async (req, res) => {
  try {
    ...
    for (let i = meses - 1; i >= 0; i--) {
      const dataReferencia = hoje.clone().subtract(i, 'months');
      const mes = dataReferencia.month() + 1; // 0-11 → 1-12
      const ano = dataReferencia.year();
      
      const startDateStr = `${ano}-${String(mes).padStart(2, '0')}-01`;
      const lastDay = new Date(ano, mes, 0).getDate();
      const endDateStr = `${ano}-${String(mes).padStart(2, '0')}-${lastDay}`;
      
      // Calcular receitas do mês
      const receitas = await Payment.aggregate([
        {
          $match: {
            $or: [
              {
      ...
      // Calcular convênios a receber (produção do mês)
      const aReceber = await Payment.aggregate([
        {
          $match: {
            paymentMethod: 'convenio',
      ...
      historico.push({
        mes: dataReferencia.format('MMM/YYYY'),
        mesNumero: mes,
        ano: ano,
        caixa: receitas[0]?.total || 0,
    ...
    res.json({
      ...
      periodo: {
        meses: meses,
        de: historico[0]?.mes,
        ate: historico[historico.length - 1]?.mes
      }
    });
    
  } catch (error) {
    console.error('[Cashflow] Erro ao buscar histórico:', error);
    res.status(500).json({
      success: false,
      message: 'Erro ao buscar histórico de caixa',
      error: error.message
    });
  }
});
